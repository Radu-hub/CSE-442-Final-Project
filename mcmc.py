...
# def log_likelihood(p, heads_count, tails_count):
#     # Using log to handle small numbers and prevent underflow.
#     # log L(p) = heads_count * log(p) + tails_count * log(1 - p)
#     # Clip p to avoid log(0) = -inf
#     eps = 1e-12
#     p = np.clip(p, eps, 1 - eps)
#     return heads_count * np.log(p) + tails_count * np.log(1 - p)


# def propose_new_state(p_current, proposal_width=0.05):
#     """
#     Given the current p, propose a new one by sampling from a normal distribution
#     centered at p_current with a standard deviation of proposal_width. 
#     If it goes out of [0,1], we 'clip' it just to keep it in range.
#     """
#     p_proposal = p_current + np.random.normal(loc=0.0, scale=proposal_width)
#     # Clip to [0, 1]:
#     p_proposal = np.clip(p_proposal, 0.0, 1.0)
#     return p_proposal

# def mh_chain(n_steps, heads_count, tails_count, proposal_width=0.05, start_p=0.5):
#     """
#     Run Metropolis-Hastings chain for 'n_steps' given coin-flip data.
