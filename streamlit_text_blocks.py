...
def text_block_2():
    ...
    st.write("We'll begin with some explanations. Options are a financial contract that give the buyer the *right* (but not the obligation! hence the name \"option\") to buy or sell an asset at a specific price, the **strike price**, before the option's **expiration date**. There are two main kinds of options: **call options**, which let you buy an asset at the strike price before the expiration date, and **put options**, which let you sell an asset at the strike price before the expiration date.")
    st.write("For example, let's say you want to buy a call option with a strike price of \$110 that expires in 1 month on Pepsi stock, which is currently worth \$100. How would you price this option? If you pay \$10 for the option, you are hoping that Pepsi stock will be worth more than \$110 in 1 month, so that you can buy it at \$110 and sell it for more than you paid for the option.")
    st.write("Say we buy this option for \$10. If Pepsi stock goes up to \$135 in 1 month, you can use the option to buy it at \$110 and then sell it for \$135, netting you a profit of \$135 (what you sell it for) - \$110 (the strike price you buy at) - \$10 (the price of the option) = \$15. You can see that the option is now worth \$15, so it is a good deal!")
    st.write("However, assume that Pepsi stock goes down to \$90 in 1 month. If you bought the option, you would not use it, because you could just buy Pepsi stock for \$90 - there's no reason to buy it at the option's strike price of \$110. In this case, the option is worthless, and you would have lost the \$10 you paid for it.")
    ...
    st.write("Pricing options is important for both buyers and sellers of options. For buyers, it allows them to figure out if an option is a good deal and thus whether they can make a profit on it. For sellers, it allows them to figure out how much to charge for an option, i.e. the 'fair price'.")


def text_block_3():
    # Monte-Carlo Simulation.
    st.write("# Monte Carlo Simulation with Metropolis-Hastings")
    st.write("In our Pepsi example, it's easy to see that the option is worth \$15, so we should buy it. However, in the real world, we don't know the exact value of the option. Instead, we can use a Monte Carlo simulation to estimate the value of the option.")
